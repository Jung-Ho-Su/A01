...
# 1. 파일 불러오기 (사용자가 새로 업로드한 24년도 승차인원 파일)
...
# 혹시 딕셔너리에 없는 호선이 있다면 계산할 수 없으므로 제외
df = df.dropna(subset=['칸수'])

# 3. 시간대 컬럼들만 추출
# 💡 24년도 데이터는 컬럼명이 '구분'이므로 이를 제외하는 조건을 추가했습니다.
...
output_filename = "서울교통공사_24년도_시간대별_추정혼잡도(%).csv"
