...
# 1. 엑셀 원본 파일 불러오기 (상단 두 줄 건너뛰기)
file_name = '송도컨벤시아 행사일정(2023).xlsx'
...
# [월별 데이터 추출] 1월 ~ 12월
for i in range(1, 13):
    # 해당 월의 행사명 중 중복을 제거한 리스트 추출
    names = df[df['개최월'] == i]['행사명'].dropna().unique().tolist()
    count = len(names)
    # 리스트의 첫 번째 값은 '개수', 두 번째부터는 '행사명'이 오도록 합침
    result_dict[f'{i}월'] = [count] + names

# [기간별 데이터 추출] N일 개최 건수
# 데이터에 존재하는 개최 일수만 오름차순으로 정렬하여 가져옴
...
# 6. 컬럼마다 행사명 개수가 다르므로, 가장 많은 행사명을 가진 컬럼 길이에 맞춰 빈칸(NaN) 채우기
max_len = max(len(v) for v in result_dict.values())
for k in result_dict.keys():
    # 부족한 길이만큼 NaN(빈 데이터) 추가
    result_dict[k] += [np.nan] * (max_len - len(result_dict[k]))

# 7. 데이터프레임으로 변환 후 CSV 파일로 저장
final_df = pd.DataFrame(result_dict)
output_filename = '2023년도 전시회 요약.csv'

# index=False 로 행 번호 생략, encoding='utf-8-sig' 로 한글 깨짐 방지
