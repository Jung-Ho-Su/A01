...
def extract_daejeon_auto_companies():
    # 1. 처리할 대전광역시 7개 파일 목록
    files = [
        "대전광역시 대덕구_제조업공장등록현황_20250818.csv",
        "대전광역시 동구_제조업 현황_20250711.csv",
        "대전광역시 서구_공장등록 현황_20260202.csv",
        "대전광역시 서구_광업 및 제조업 현황_20231231.csv",
        "대전광역시 유성구_공장등록 현황_20250512.csv",
        "대전광역시 중구 제조업체 현황_20250110.csv",
        "대전광역시_공장현황_20221231.csv"
    ]
    ...
    for file in files:
        ...
        if not filtered_df.empty:
            # 4. 대전 지자체별로 제각각인 컬럼명을 공통 컬럼명으로 통일
            ...
            # [주소] 통일 ('공장대표주소', '도로명주소', '소재지', '소재지(도로명)')
            addr_cols = ['공장대표주소', '공장대표주소(도로명)', '도로명주소', '주소', '소재지주소', '소재지', '소재지(도로명)']
            filtered_df['통합_주소'] = ""
            ...
            # 6. 의미 없는 데이터 정리 (회사명이 비어있거나 숫자만 있는 경우 등 통계 파일 필터링)
    ...
    if extracted_dataframes:
        ...
        # 저장할 파일명 지정
        output_file = "대전광역시_자동차산업_관련_회사목록_통합본.csv"
...
if __name__ == "__main__":
    extract_daejeon_auto_companies()