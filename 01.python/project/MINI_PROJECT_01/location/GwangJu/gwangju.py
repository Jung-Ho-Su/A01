...
def extract_gwangju_auto_companies():
    # 1. 처리할 광주광역시 4개 파일 목록
    files = [
        "광주광역시 광산구_제조업체 현황_20250910.csv",
        "광주광역시 남구_제조업공장_20250904.csv",
        "광주광역시_자동차산업 관련 일반현황_20221231.csv",
        "광주광역시_제조업체 현황_20230201.csv"
    ]
    ...
    for file in files:
        ...
        # 광주 데이터 중 열 이름에 공백이 섞인 경우(' 업종 ' 등)를 대비해 공백 제거
        ...
        if not filtered_df.empty:
            # 4. 광주 지자체별로 제각각인 컬럼명을 공통 컬럼명으로 통일
            ...
            # [주소] 통일 ('소재지', '소재지(도로명)' 등 반영)
            addr_cols = ['소재지(도로명)', '소재지', '공장대표주소', '도로명주소', '주소']
            filtered_df['통합_주소'] = ""
            ...
            # 6. 의미 없는 통계용 데이터 제거
            # 회사명이 비어있거나, 숫자로만 되어있거나, 특정 통계 단어가 있는 경우 필터링
    ...
    if extracted_dataframes:
        ...
        # 저장할 파일명 지정
        output_file = "광주광역시_자동차산업_관련_회사목록_통합본.csv"
...
if __name__ == "__main__":
    extract_gwangju_auto_companies()