...
def extract_chungnam_auto_companies():
    # 1. 처리할 충청남도 10개 파일 목록
    files = [
        "충청남도_부여군_제조업체(공장) 등록현황_20260126.csv",
        "충청남도 계룡시_제조업체_10_29_2021.csv",
        "충청남도 서천군_기업체 현황_20250712.csv",
        "충청남도 아산시_제조업 공장등록 현황_20250430.csv",
        "충청남도 예산군_산업단지 현황_20230915.csv",
        "충청남도 예산군_제조업체 등록현황_20230824.csv",
        "충청남도 제조업체 현황_20250903.csv",
        "충청남도 천안시_공장(제조업체)현황_20251128.csv",
        "충청남도 청양군_제조업체현황_20251105.csv",
        "충청남도 청양군_지역별 제조업체 현황_20250901.csv"
    ]
    ...
    for file in files:
        ...
        if not filtered_df.empty:
            # 4. 충남 지자체별로 제각각인 컬럼명을 공통 컬럼명으로 통일

            # [회사명] 통일 ('회사명', '업체명', '기업체명', '입주업체명', '상호명')
            company_cols = ['회사명', '업체명', '기업체명', '입주업체명', '상호명']
            filtered_df['통합_회사명'] = ""
            ...
            # [주소] 통일 ('공장대표주소', '공장대표주소(도로명)', '도로명주소', '주소', '소재지')
            addr_cols = ['공장대표주소', '공장대표주소(도로명)', '도로명주소', '주소', '소재지주소', '소재지', '공장대표주소(지번)']
            filtered_df['통합_주소'] = ""
    ...
    if extracted_dataframes:
        ...
        # 저장할 파일명 지정
        output_file = "충청남도_자동차산업_관련_회사목록_통합본.csv"
...
if __name__ == "__main__":
    extract_chungnam_auto_companies()